...
import type { DashboardData } from "@/lib/db/audits";
...
export function SummaryClient({ data }: { data: DashboardData | null }) {
  const identity = useIdentity();
  const firstName = identity.displayName.split(/\s+/)[0];
  const isGuest = identity.isGuest;
  const isEmptyAuthed = !isGuest && (!data || data.totalAudits === 0);

  return (
    <>
      <PageHeader
        title="My Summary"
        subtitle={buildSubtitle(isGuest, firstName, identity.stats)}
        actions={
          <button
            type="button"
      ...
      >
        ...
        >
          {isGuest ? (
            ...
            <ComparisonCard
              thisMonth={data?.thisMonthRecovered ?? 0}
              lastMonth={data?.lastMonthRecovered ?? 0}
              delta={data?.monthDelta ?? 0}
              empty={isEmptyAuthed}
            />
          )}
          {isGuest ? (
            ...
            <AuditTrendCard
              recentAudit={data?.recentAudit ?? null}
              weeklyValues={data?.weeklyTrend}
              mostFlagged={data?.mostFlagged.map((f) => ({
                code: f.code,
                percent: f.percent,
              }))}
              topFacility={data?.topFacility ?? null}
              empty={isEmptyAuthed}
            />
          )}
        </motion.div>
        ...
        <motion.div variants={fadeIn} transition={{ duration: 0.5, ease: EASE }}>
          {isGuest ? (
            ...
            <GoalsCard
              thisMonth={data?.thisMonthRecovered ?? 0}
              lastMonth={data?.lastMonthRecovered ?? 0}
              byStatute={data?.byStatute.map((s) => ({
                code: s.code,
                percent: s.percent,
              }))}
              empty={isEmptyAuthed}
